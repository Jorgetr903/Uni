...
class SimpleModel(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(2, 6)  # Capa densa con 6 unidades de entrada y 2 unidades de salida
        self.fc2 = nn.Linear(6, 6)  # Capa densa con 6 unidades de entrada y 6 unidades de salida

    def forward(self, x):
        x = torch.relu(self.fc1(x))  # Aplicar activación ReLU a la salida de la primera capa
        x = self.fc2(x)  # La capa final no necesita activación ya que se usa CrossEntropyLoss
        return x
...
# Entrenar el modelo
train_losses, val_losses = train_model(simple_model, criterion, optimizer, X_train_tensor, y_train_tensor, X_cv_tensor, y_cv_tensor, epochs=1000)
