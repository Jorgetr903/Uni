...
public:
	...
	linked_list_ed() : prim(nullptr), ult(nullptr), ultPrioritario(nullptr) {}
	...
	void push_back_prioritario(T const& elem) {
		Nodo* nuevo = new Nodo(elem);
		...
		{
			prim = nuevo;
			ultPrioritario = nuevo;
			ult = nuevo;
		}
		else if (ultPrioritario == nullptr) { // lista no vacia pero no hay ningún prioritario aún
			ultPrioritario = nuevo;
			ultPrioritario->sig = prim;
			prim = nuevo;
			if (ultPrioritario->sig == nullptr) {
				ult = nuevo;
			}
		}
		else
		{
			Nodo* aux = ultPrioritario; // existen prioritarios
			ultPrioritario = nuevo;
			ultPrioritario->sig = aux->sig;
			aux->sig = nuevo;
			if (ultPrioritario->sig == nullptr) {
				ult = nuevo;
			}
	...
	void pop_front() {
		if (empty())
			throw std::domain_error("Cola vacia");
		Nodo* a_borrar = prim;
		if (a_borrar == ultPrioritario) {
