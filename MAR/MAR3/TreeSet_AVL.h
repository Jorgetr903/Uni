...
public:
    ...
    std::list<T> elementosEntre(T const& min, T const& max) const {
        std::list<int> resultado;
        elementosEntre(min, max, raiz, resultado);
        return resultado;
    }

    void elementosEntre(int const& min, int const& max, Link nodo, std::list<int>& resultado) const {
        if (nodo == nullptr) {
            return;
        }

        if (menor(min, nodo->elem)) {
            elementosEntre(min, max, nodo->iz, resultado);
        }

        if (!menor(nodo->elem, min) && !menor(max, nodo->elem)) {
            resultado.push_back(nodo->elem);
        }

        if (menor(nodo->elem, max)) {
            elementosEntre(min, max, nodo->dr, resultado);
        }
    }
